...
Staticman.prototype._generateReviewBody = function (fields, options) {
  let table = [
    ['Field', 'Content']
  ]
  ...
  let message = this._resolvePlaceholders(this.siteConfig.get('pullRequestBody'), {
    fields,
    options,
  })
  ...
  return this.getSiteConfig().then(config => {
    ...
    if (this.siteConfig.get('moderation')) {
      ...
      return this.git.writeFileAndSendReview(
        ...
        newBranch,
        commitMessage,
        this._generateReviewBody(fields, options)
      )
